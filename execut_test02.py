...
SECTIONS_INFO = [
        {'case': 1, 'params_sections': [{'length': 15.24,
        'elevation': 1.12, 'angle': 0.07}]},
        {'case': 2, 'params_sections': [{'length': 15.24,
        'elevation': -1.14, 'angle': -0.08}]},
        {'case': 3, 'params_sections': [{'length': 15.24,
        'elevation': 1.86, 'angle': 0.12}]},
        {'case': 4, 'params_sections': [{'length': 15.24,
        'elevation': -1.01, 'angle': -0.07}]},
        {'case': 5, 'params_sections': [
            {'length': 30.48, 'elevation': 0.07, 'angle': -0.002},
            {'length': 14.02, 'elevation': 2.01, 'angle': 0.14}
        ]},
        {'case': 6, 'params_sections': [{'length': 16.46,
        'elevation': -1.69, 'angle': -0.1}]},
    ]
...
def create_my_parameters_reference(table_row):
    ...
    return multiphase_equations.PhysicalChemicalProperties(
        ql=F,
        qg=Fgas,
        D=0.052,
        e = 0,
...
def funcao_de_conversao_de_unidade(P_psia, T_F, F_BD, Fgas_scfD):
    ...
    Fgas = Fgas_scfD * 2.863640 * (10 ** - 2) * 351.82 * T / (
        24 * 3600 * P)

    return P, T, F, Fgas

def calc_dP_sections_single_row(table_row, SECTIONS_INFO,
expected_Pdrop=None, print_calcs=False):
    ...
    for i in range(npts):
        ...
        for j in range(len(row['params_sections'])):
            ...
            L = length
            calculator_bb = multiphase_equations.BeggAndBrillCalculator(
                my_params_iteration)
        ...
        if print_calcs:
            ...
            print(
                '[{}] Calculated dPf Section {} = {:.2f} (expected = {:.2f}; {:.2f}%)'
                .format(
...
def main_calc_dPf_with_the_class_single_row():
    ...
    P, T, F, Fgas = funcao_de_conversao_de_unidade(P_psia, T_F,
    F_BD, Fgas_scfD)
...
def calc_Hl_sections_single_row(table_row, SECTIONS_INFO,
expected_Hl=None, print_calcs=False):
    npts = len(SECTIONS_INFO)
    ...
    for i in range(npts):
        ...
        if print_calcs:
            ...
            print(
                '[{}] Calculated Hl Section {} = {:.2f} (expected = {:.2f}; {:.2f}%)'
                .format(
                'asc' if angle > 0 else 'desc',
...
def main_calc_Hl_with_the_class_single_row():
    ...
    Hl_5 = row["s5b"]
    Hl_6 = row["s6"]
    P, T, F, Fgas = funcao_de_conversao_de_unidade(
        P_psia, T_F, F_BD, Fgas_scfD)

    table_row = {'P': P, 'T': T, 'F': F, 'Fgas': Fgas}

    expected_Hl = np.array([Hl_1, Hl_2, Hl_3, Hl_4, Hl_5, Hl_6])

    Hl_calcs = calc_Hl_sections_single_row(table_row,
    SECTIONS_INFO, expected_Hl)
