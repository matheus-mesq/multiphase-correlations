...
class PhysicalChemicalProperties():
    def __init__(self, ql = 0.001058, qg = 0.01649,
        D = 0.052, e = 0, dl = 999.55, ML = 0.018, MG = 0.016, ts = 0.06, g = 9.8,
        ang = 0.07, z = 1.12, L = 15.24, T = 296.483, P0 = 2937167.76,
        z_gas=1.0, Dp = 0.06, Di = 0.07, kp = 50, ki = 0.2, he = 10, Te = 276.15,
...
def calc_mil(params):
    "Esta função calcula a viscosidade do líquido(Pa * s). Equação não utilizada."
    dl = params.dl
    T = params.T
    API = (141.5 / (dl / 1000)) - 131.5
    a = 10 ** (0.43 + (8.33 / API))
    mil = (0.32 + (1.8 * (10 ** 7) / (API ** 4.53))) * ((360 / (T + 200)) ** a) * 0.001
    return mil
...
def calc_mig(dg, params):
    ...
    MG = params.MG
    T = params.T
    K = ((9.4 + (0.02 * MG * 1000)) * ((9 * T / 5) ** 1.5)) / (209 + (19 * MG * 1000) + (9 * T / 5))
...
def calc_vsl(Ap, params):
    "Esta função calcula a velocidade superficial do líquido(m / s). Eq. 2-11."
    ql = params.ql
    vsl = ql / Ap
    return vsl

def calc_vsg(Ap, params):
    "Esta função calcula a velocidade superficial do gás(m / s). Eq. 2-12."
    qg = params.qg
    vsg = qg / Ap
    return vsg
...
def calc_min(mil, mig, Cl):
    "Esta função calcula a viscosidade de não-escorregamento(N x m / s²). Eq. 2-6."
    min = (mil * Cl) + (mig * (1 - Cl))
    return min

def calc_dn(dg, Cl, params):
    "Esta função calcula a densidade de não-escorregamento(kg / m³). Eq. 2-8."
    dl = params.dl
    dn = (dl * Cl) + (dg * (1 - Cl))
    return dn
...
def calc_Nfr(vm, params):
    "Esta função calcula o número de Froude de mistura. Eq. 2-52."
    D = params.D
    g = params.g
    Nfr = (vm ** 2) / (g * D)
    return Nfr
...
def calc_Hl0(Cl, L1, L2, L3, L4, Nfr):
    "Esta função calcula a fração volumétrica in-situ para o tubo na posição horizontal. Eq. 2-51."
    if (Cl < 0.01 and Nfr < L1) or (Cl >= 0.01 and Nfr < L2): # Segregado
        Hl0 = 0.98 * (Cl ** 0.4846) * (Nfr ** - 0.0868)
    elif (0.01 <= Cl < 0.4 and L3 < Nfr <= L1) or (Cl >= 0.4 and L3 < Nfr <= L4): # Intermitente
...
def calc_Nlv(vsl, params):
    "Esta função calcula o número de velocidade do líquido. Eq. 2-55."
    dl = params.dl
    g = params.g
    ts = params.ts
...
def detect_multiphase_regime(Cl, L1, L2, L3, L4, Nfr) -> MultiphaseFlowType:
    ...
    elif (Cl < 0.4 and Nfr >= L1) or (Cl >= 0.4 and Nfr > L4): # Distribuído
...
def calc_C(Cl, L1, L2, L3, L4, Nfr, Nlv, params):
    ...
    if ang > 0:
        if (Cl < 0.01 and Nfr < L1) or (Cl >= 0.01 and Nfr < L2): # Segregado
            C = (1 - Cl) * np.log(0.011 * (Cl ** - 3.768) * (Nfr ** - 1.614) * (Nlv ** 3.539))
        elif (0.01 <= Cl < 0.4 and L3 < Nfr <= L1) or (Cl >= 0.4 and L3 < Nfr <= L4): # Intermitente
            C = (1 - Cl) * np.log(2.96 * (Cl ** 0.305) * (Nfr ** 0.0978) * (Nlv ** - 0.4473))
        else: # Distribuído
            C = 0
    else:
        C = (1 - Cl) * np.log(4.7 * (Cl ** - 0.3692) * (Nfr ** - 0.5056) * (Nlv ** 0.1244))
    return C

def calc_B(C, params):
    "Esta função calcula o fator de inclinação. Eq. 2-53."
    ang = params.ang
    B = 1 + (C * ((np.sin(1.8 * ang)) - (((np.sin(1.8 * ang)) ** 3) / 3)))
    return B

def calc_Hl(Cl, L2, L3, Nfr, Nlv, B, Hl0, params):
    "Esta função calcula a fração volumétrica in-situ para o ângulo de inclinação do tubo. Eq. 2-50."
    ang = params.ang
    if Cl>= 0.01 and L2 <= Nfr <= L3: # Transição
        Hlseg = (1 + (((1 - Cl) * np.log(0.011 * (Cl ** - 3.768) * (Nfr ** - 1.614) * (Nlv ** 3.539))) * ((np.sin(1.8 * ang)) - (((np.sin(1.8 * ang)) ** 3) / 3)))) * 0.98 * (Cl ** 0.4846) * (Nfr ** - 0.0868)
        Hlint = (1 + (((1 - Cl) * np.log(2.96 * (Cl ** 0.305) * (Nfr ** 0.0978) * (Nlv ** - 0.4473))) * ((np.sin(1.8 * ang)) - (((np.sin(1.8 * ang)) ** 3) / 3)))) * 0.845 * (Cl ** 0.5351) * (Nfr ** - 0.0173)
        A = (L3 - Nfr) / (L3 - L2)
...
def fun_Hl(params):
    """Esta função calcula a fração volumétrica in-situ para o ângulo de inclinação do tubo,
    utilizando somente os parâmetros como argumento.
    """
    Ap = calc_Ap(params)
    Cl = calc_Cl(params)
...
def calc_dtp(dg, Hl, params):
    "Esta função calcula a densidade de mistura(kg / m³). Eq. 2-7."
    dl = params.dl
    dtp = (dl * Hl) + (dg * (1 - Hl))
    return dtp

def calc_dPedL(dtp, params):
    "Esta função calcula o gradiente de pressão por elevação(Pa / m). Eq. 2-70."
    ang = params.ang
    g = params.g
...
def calc_fn(Re, params):
    "Esta função calcula o fator de atrito de não-escorregamento. Eqs. 2-60 e 2-61."
    D = params.D
    e = params.e
    er = e / D
    if Re < 2100: # Laminar
        ...
        fn = ((1) / (- 2 * np.log10((er / 3.7065) - ((5.0452 / Re) * np.log10(((er ** 1.1098) / 2.8257) + (5.8506 / (Re ** 0.8981))))))) ** 2
    return fn
...
def calc_dPadL(ftp, dn, vm, params):
    "Esta função calcula o gradiente de pressão por atrito(Pa / m). Eq. 2-69."
    D = params.D
...
def calculate_dPdL(params):
    "Esta função calcula o gradiente de pressão utilizando somente os parâmetros como argumento(Pa / m)."
...
class BeggAndBrillCalculator():
    ...
    def calculate_dPdL(self) -> float:
        ...
        self.flow_type = detect_multiphase_regime(self.Cl, self.L1, self.L2, self.L3, self.L4, self.Nfr)
        self.Hl0 = calc_Hl0(self.Cl, self.L1, self.L2, self.L3, self.L4, self.Nfr)
        self.Nlv = calc_Nlv(self.vsl, params)
        self.C = calc_C(self.Cl, self.L1, self.L2, self.L3, self.L4, self.Nfr, self.Nlv, params)
        self.B = calc_B(self.C, params)
        self.Hl = calc_Hl(self.Cl, self.L2, self.L3, self.Nfr, self.Nlv, self.B, self.Hl0, params)
        ...
        self.dPdL = calc_dPdL(self.dPadL, self.dPedL, self.dtp, self.vm, self.vsg, params)
        return self.dPdL
...
def calc_U(params):
    "Esta função calcula o coeficiente global de transferência de calor(W/m² K)."
    D = params.D
    Dp = params.Dp
    Di = params.Di
    kp = params.kp
    ki = params.ki
    he = params.he
    U = (2 / D) * ((((np.log(Dp / D)) / kp) + ((np.log(Di / Dp)) / ki) + (2 / (he * Di))) ** - 1)
    return U

def calc_Qf(U, params):
    "Esta função calcula o fluxo de calor(W/m²)."
    T = params.T
    Te = params.Te
    Qf = U * (T - Te)
    return Qf
...
def calc_Sp(params):
    "Esta função calcula o perímetro do tubo(m)."
    D = params.D
    Sp = np.pi * D
    return Sp

def calc_Wm(dg, params):
    "Esta função calcula a vazão mássica da mistura(kg/s)."
    ql = params.ql
    qg = params.qg
    dl = params.dl
    Wl = dl * ql
...
def calc_dHdL(Wm, Sp, Qf, params):
    "Esta função calcula o gradiente de entalpia específica(J/kg m)."
    g = params.g
    ang = params.ang
    dHdL = (Sp * Qf / Wm) - (g * np.sin(ang))
    return dHdL

def calc_H_m_agua(params):
    "Esta função calcula a entalpia molar da água(J/mol)."
    A = - 2.46342
    B = 0.457392
    C = - 0.052512 * (10 ** - 3)
    D = 0.064594 * (10 ** - 6)
    ...
    T = params.T
    ML = params.ML
    H_m_agua = 2.326 * (A + (B * 9 * T / 5) + (C * ((9 * T / 5) ** 2)) + (D * ((9 * T / 5) ** 3)) + (E * ((9 * T / 5) ** 4)) + (F * ((9 * T / 5) ** 5))) * ML
    return H_m_agua

def calc_H_m_gas(params):
    "Esta função calcula a entalpia molar do gás(J/mol)."
    A = - 5.58114
    B = 0.564834
    C = - 0.282973 * (10 ** - 3)
    D = 0.417399 * (10 ** - 6)
    ...
    T = params.T
    MG = params.MG
    H_m_gas = 2.326 * (A + (B * 9 * T / 5) + (C * ((9 * T / 5) ** 2)) + (D * ((9 * T / 5) ** 3)) + (E * ((9 * T / 5) ** 4)) + (F * ((9 * T / 5) ** 5))) * MG
    return H_m_gas

def calc_Beta(dg, params):
    "Esta função calcula a fração molar vaporizada"
    ql = params.ql
    dl = params.dl
    ML = params.ML
    qg = params.qg
...
def calc_H_e(H_m_agua, H_m_gas, Beta, params):
    "Esta função calcula a entalpia específica da mistura(J/kg)."
    ML = params.ML
    MG = params.MG
    z_agua = 1 - Beta
    z_gas = Beta
    H_m = (H_m_agua * (1 - Beta)) + (H_m_gas * Beta)
    M = (z_agua * ML) + (z_gas * MG)
    H_e = H_m / M
    return H_e
